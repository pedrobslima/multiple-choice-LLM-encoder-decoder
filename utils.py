...
def cross_entropy_list(sources, targets, model, tokenizer=None, cache=None, batch=False, calculate=True):
    """
    Wrapper function that maintains compatibility with original interface
    but routes to appropriate implementation based on model type
    """

    # Check if this is a T5 model
    if hasattr(model, 'encoder') and hasattr(model, 'decoder'):
        # This is likely a T5 model
        if tokenizer is None:
            raise ValueError("Tokenizer is required for T5 models")
        return cross_entropy_list_t5(sources, targets, model, tokenizer, cache, batch, calculate)
    else:
        # Fallback to original GPT-2 style implementation
        return cross_entropy_list_gpt2(sources, targets, model, cache, batch, calculate)

# ===================================================

def cross_entropy_list_gpt2(sources, targets, model, cache=None, batch=False, calculate=True):
    """
    Original GPT-2 implementation (kept for backward compatibility)
    """
    ...
    device = next(model.parameters()).device #model.transformer.wte.weight.device
...
def cross_entropy_list_t5(sources, targets, model, tokenizer, cache=None, batch=False, calculate=True):
    """
    Gets a list of CE values for T5 model, where the ith item is cross-entropy
    for targets[i] with sources[i] as contexts

    For T5 (encoder-decoder model):
    - sources are the input contexts (encoded by encoder)
    - targets are the expected outputs (decoded by decoder)

    Args:
        sources: List of source texts or token lists
        targets: List of target texts or token lists
        model: T5 model (T5ForConditionalGeneration)
        tokenizer: T5 tokenizer
        cache: Dictionary for caching results
        batch: Batch size or False for no batching
        calculate: Whether to calculate immediately or defer
    """

    ###############################
    # CACHING LOGIC (same as original)
    if cache is not None:
        # log calculations we have not done yet
        for source, target in zip(sources, targets):
            key = get_key(source, target)
            if key not in cache:
                cache[key] = {'source': source, 'target': target, 'result': None}
        ...
        if len(cache_todo) > 0:
            ...
            cache_results = cross_entropy_list_t5(
                sources_todo, targets_todo, model, tokenizer,
    ...
    if batch:
        ...
        for i in tqdm(list(range(n_batches))):
            batch_start = i * batch
            batch_end = (i + 1) * batch
            ce_list += cross_entropy_list_t5(
                sources[batch_start:batch_end],
                targets[batch_start:batch_end],
                model, tokenizer, batch=False
            )
        return ce_list

    # Convert sources and targets to proper format for T5
    processed_sources = []
    processed_targets = []

    for source, target in zip(sources, targets):
        # Handle both token lists and text strings
        if isinstance(source, list):
            # If source is token list, decode to text
            source_text = tokenizer.decode(source, skip_special_tokens=True)
        else:
            source_text = str(source)

        if isinstance(target, list):
            # If target is token list, decode to text
    ...
    # Tokenize inputs for T5 encoder-decoder
    # For T5, we tokenize source and target separately
    ...
    # Calculate cross-entropy using T5 model
    with torch.no_grad():
        outputs = model(
            input_ids=input_ids,
        ...
        # Get logits and calculate per-example loss
        logits = outputs.logits  # Shape: (batch_size, target_seq_len, vocab_size)

        # Calculate cross-entropy for each example individually
        ce_list = []

        for i in range(n_seqs):
            # Get logits and labels for this example
            example_logits = logits[i]  # (target_seq_len, vocab_size)
            example_labels = labels[i]   # (target_seq_len,)

            # Only calculate loss for non-ignored tokens
            valid_mask = (example_labels != -100)

            if valid_mask.sum() > 0:  # If there are valid tokens
                valid_logits = example_logits[valid_mask]  # (valid_tokens, vocab_size)
                valid_labels = example_labels[valid_mask]  # (valid_tokens,)

                # Calculate cross-entropy for this example
                example_ce = F.cross_entropy(valid_logits, valid_labels, reduction='sum')
                ce_list.append(example_ce.item())
            else:
                # If no valid tokens, set CE to 0
                ce_list.append(0.0)

    # Handle single element case (same as original)
    if len(ce_list) == 1:
        return ce_list
...
def inference_autobatch(model, encoder, example, batch = 1, prelog = False, cache = None):
    ...
    max_len = 1024 # TODO: mudar o max_len?
    #gpt3 = False
...
#
    ...
    cond_ce = cross_entropy_list([opt['premise'] for opt in options],
                                [opt['hypothesis'] for opt in options],
                                model, tokenizer=encoder, cache=cache, batch=batch, calculate = calculate) # TODO!!!!
...
def fwd(model, encoder, examples, batch, cache = None):
    ...
    if cache is not None:
        ...
        for example in tqdm( examples):
            _ = inference_autobatch(model, encoder, example, prelog=True, cache = cache, batch=batch ) # TODO!!!
    ...
    for example in tqdm(examples):
        pred = inference_autobatch(model, encoder, example, prelog=False, cache = cache, batch=batch ) # TODO!!!
...
def score(model, model_name, encoder, examples, stem, split, batch):
    ...
    accs, preds = fwd(model, encoder, examples, batch, cache) # TODO
